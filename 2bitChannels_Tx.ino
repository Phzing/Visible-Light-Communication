...
uint8_t bitsToLEDVal(const char* stream, int start) {
  if (stream[start] == '0') {
    if (stream[start+1] == '0') {
      return 0;
    }
...
void setup() {
  ...
  for (int frame = 0; frame < NUM_FRAMES; frame++) {
    ...
    for (int i = 0; i < BLOCKS_PER_FRAME; i++) {
      ...
      int bitStart = frameStart + i * BITS_PER_BLOCK;
      uint16_t color = matrix.color565(bitsToLEDVal(bitStream, bitStart), bitsToLEDVal(bitStream, bitStart+2), bitsToLEDVal(bitStream, bitStart+4));
    ...
    for (int j = CLOCK_BLOCK; j < BLOCKS_PER_FRAME+1; j++) {
      int bitStart = frameStart + (j-1) * BITS_PER_BLOCK;
